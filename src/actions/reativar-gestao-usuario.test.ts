...
import { reativarGestaoUsuarioAction } from "./reativar-gestao-usuario";
...
describe("reativarGestaoUsuarioAction", () => {
    ...
    it("deve reativar o usuário com sucesso", async () => {
        cookiesMock.mockReturnValue({
            get: vi.fn().mockReturnValue({ value: mockToken }),
        });
        apiPostMock.mockResolvedValueOnce({} as never);

        const result = await reativarGestaoUsuarioAction(uuid);
