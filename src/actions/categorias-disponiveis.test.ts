...
import { getCategoriasDisponiveisAction, CategoriasDisponiveisAPI } from "./categorias-disponiveis";
...
describe("getCategoriasDisponiveisAction", () => {
  ...
  it("deve retornar dados corretamente quando a API retorna sucesso", async () => {
    cookiesMock.mockReturnValue({ get: vi.fn().mockReturnValue({ value: mockAuthToken }) });
  ...
  it("deve retornar erro 401 para não autorizado", async () => {
    cookiesMock.mockReturnValue({ get: vi.fn().mockReturnValue({ value: mockAuthToken }) });
    axiosGetMock.mockRejectedValueOnce({ response: { status: 401 } } as AxiosError);

    const result = await getCategoriasDisponiveisAction();

    expect(result).toEqual({ success: false, error: "Não autorizado. Faça login novamente." });
  });

  it("deve retornar erro 400 quando categorias não encontradas", async () => {
    cookiesMock.mockReturnValue({ get: vi.fn().mockReturnValue({ value: mockAuthToken }) });
    axiosGetMock.mockRejectedValueOnce({ response: { status: 400 } } as AxiosError);

    const result = await getCategoriasDisponiveisAction();

    expect(result).toEqual({ success: false, error: "Categorias não encontrados" });
  });

  it("deve retornar erro 500 para erro interno do servidor", async () => {
    cookiesMock.mockReturnValue({ get: vi.fn().mockReturnValue({ value: mockAuthToken }) });
    axiosGetMock.mockRejectedValueOnce({ response: { status: 500 } } as AxiosError);
  ...
  it("deve retornar erro com message genérica", async () => {
    cookiesMock.mockReturnValue({ get: vi.fn().mockReturnValue({ value: mockAuthToken }) });
    axiosGetMock.mockRejectedValueOnce({ message: "Erro desconhecido" } as AxiosError);
